...
def load_data(dir):
    ...
    for user_num in range(1, users + 1):
        ...
        data[user] = {}
        # data = {"user1":
        #           {"machine": {"transcript": [], "sentiment": [], "pitch": [], "speechrate": [], "category: []"},
        #            "history": {"transcript": [], "sentiment": [], "pitch": [], "speechrate": [], "category: []"},
        #             ...
        #            "remote": {"transcript": [], "sentiment": [], "pitch": [], "speechrate": [], "category: []"}
        #           },
        #         ...
        #         "user2":
        #           {"machine": {"transcript": [], "sentiment": [], "pitch": [], "speechrate": [], "category: []"},
        ...
        for product in products:
            ...
            for feature in features:
                data[user][product][feature] = []

    for user_num in range(1, users + 1):
        user = "user" + str(user_num)
        for product in products:
            file_lists = os.listdir(dir + '/' + user + '/' + product + '/')
            for file in file_lists:
                if file.split('.')[-1] == 'json':
                    print(dir + '/' + user + '/' + product + '/' + file)

                    with open(dir + '/' + user + '/' + product + '/' + file, 'r') as f:
                        ...
                        for elem in collection:
                            data[user][product]["transcript"].append(elem['transcription'])

                            idx = elem['problem']
                            target[user][product].append(idx)

                            category_code = category_map[elem['category']]
                            data[user][product]["category"].append(category_code)

                            data[user][product]["sentiment"].append(elem['sentiment_gt'])
                        ...
                        for i in range(len(collection)):
                            ...
                            if abnormal_cnt.count(-1) >= pitch_low_threshold:
                                pitch_code[1] = 1
                            data[user][product]["pitch"].append(pitch_code)
                            ...
                            if (seg_speechrate - avg_speechrate) <= -2 * std_speechrate:
                                speechrate_code[1] = 1
                            data[user][product]["speechrate"].append(speechrate_code)
...
def get_all_info(leave_out_product, user, data, target):
    test_target = []
    test_data = []
    test_category = []
    test_sentiment = []
    ...
    train_speechrate = []
    for product in products:
        if product == leave_out_product:
            test_target.extend(target[user][product])
            test_data.extend(data[user][product]["transcript"])
            test_category.extend(data[user][product]["category"])
            test_sentiment.extend(data[user][product]["sentiment"])
            test_pitch.extend(data[user][product]["pitch"])
            test_speechrate.extend(data[user][product]["speechrate"])
        else:
            train_target.extend(target[user][product])
            train_data.extend(data[user][product]["transcript"])
            train_category.extend(data[user][product]["category"])
            train_sentiment.extend(data[user][product]["sentiment"])
            train_pitch.extend(data[user][product]["pitch"])
            train_speechrate.extend(data[user][product]["speechrate"])
...
# return exclude product and user as test, the rest of all data a training data
def get_train_test(data, target, exclude_product, exclude_user):
    test_target = []
    test_data = []
    test_category = []
    test_sentiment = []
    ...
    for user_num in range(1, users + 1):
        user = "user" + str(user_num)
        for product in products:
            if product == exclude_product and user == exclude_user:
                test_target.extend(target[user][product])
                test_data.extend(data[user][product]["transcript"])
                test_category.extend(data[user][product]["category"])
                test_sentiment.extend(data[user][product]["sentiment"])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='input a model name, and a user name, within whose data that \
    the leave one product out evaluation is performed')
    ...
    parser.add_argument('userName', type=int,
                        help='A required user name argument: integer from 1 to 8')

    args = parser.parse_args()
    modelName = args.modelName
    userName = int(args.userName)
    if modelName not in ['rf', 'svm']:
        print('input model has to be one of rf or svm')
        exit(1)

    if userName not in [1,2,3,4,5,6,7,8]:
        print('input user to leave out has to be one of integers from 1 to 8')

    # data and target initialization
    dir = './user-data/user-data-productout'
    categories = ['Observation', 'Procedure', 'Explanation', 'Reading']
    data = None
    target = None
    # no previous loaded data, load and save them
    if not (os.path.isfile('./save/save-data-productout/data.txt')
            and os.path.isfile('./save/save-data-productout/target.txt')
            ):
        print("load data")
        data, target = load_data(dir)

        with open("./save/save-data-productout/data.txt", "wb") as d:  # Pickling
            pickle.dump(data, d)
            d.close()
        with open("./save/save-data-productout/target.txt", "wb") as t:  # Pickling
            pickle.dump(target, t)
            t.close()

    # data already loaded, retrieve them from ./save/save-data/save-data
    else:
        print('retrieve data')
        with open("./save/save-data-productout/data.txt", "rb") as d:  # Pickling
            data = pickle.load(d)
            d.close()
        with open("./save/save-data-productout/target.txt", "rb") as t:  # Pickling
            target = pickle.load(t)
            t.close()

    for user in data:
        for product in data[user]:
            for sentence in range(len(data[user][product]["transcript"])):
                # otherwise "coffee" and "coffee," would be treated as different tokens
                data[user][product]["transcript"][sentence] \
                    = data[user][product]["transcript"][sentence].replace(",", "")
                data[user][product]["transcript"][sentence] \
                    = data[user][product]["transcript"][sentence].replace(".", "")
                data[user][product]["transcript"][sentence] \
                    = data[user][product]["transcript"][sentence].replace("?", "")
    ...
    print("\n\n=== Model selected: {} Current User data set being evaluated {}===".format(modelName, user))

    test_products = ["machine", "remote", "science", "history"]
    overall_acc = 0
    overall_recall = 0
    overall_precision = 0
    print('leave one product out evaluation:\n')
    for leaveOut_product_toTest in test_products:
        print('---\nuser ID:      {}'.format(user))
        print('---\ntest product: {}'.format(leaveOut_product_toTest))

        test_target, test_data, test_category, test_sentiment, test_pitch, test_speechrate, \
        train_target, train_data, train_category, train_sentiment, train_pitch, train_speechrate \
            = get_all_info(leaveOut_product_toTest, user, data, target)
        ...
        file_lists = os.listdir(dir + '/' + user + '/' + leaveOut_product_toTest + '/')
        # one user could contain multiple files, but prediction outputs all of them in one array
        offset = 0
        for file in file_lists:
            if file.split('.')[-1] == 'json':

                with open(dir + '/' + user + '/' + leaveOut_product_toTest + '/' + file, 'r') as f:
                    collection = JSON.load(f, object_pairs_hook=OrderedDict)
                f.close()
                with open(dir + '/' + user + '/' + leaveOut_product_toTest + '/' + file, 'w') as out:
                    for i in range(len(collection)):
